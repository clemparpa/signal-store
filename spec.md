...
- **`toSignal(observable, initial)`** : inverse de `toObservable` (cf. §4.10). Signature de base hors-store : `toSignal<T>(source: Observable<T>, initial: T): readonly [ReadonlySignal<T>, () => void]` — tuple `[signal, dispose]` à la `useState`, le caller libère manuellement la sub (le `dispose` est juste un wrapper sur `Subscription.unsubscribe`, nativement idempotent en RxJS). Surcharge store-aware : `toSignal<T>(store: object, source: Observable<T>, initial: T): ReadonlySignal<T>` — sub ajoutée à `meta.cleanup`, auto-libérée par `destroyStore` ou à l'unmount du Provider React, donc pas de `dispose` exposé (le store gère le cycle de vie). Vit dans `@fluch/signal-store` à côté de `toObservable` (rxjs est déjà peer dep du core depuis §5.5). Comportement : `signal.value = initial` au call, chaque `next` push une nouvelle valeur. `error` Observable → `console.error`, le signal garde la dernière valeur (Observables infinis dans la lib ne devraient pas erroriser ; on n'introduit pas un `errorSignal` séparé). `complete` → on garde la dernière valeur et on libère la sub interne. **Décision tuple vs object intersection (`ReadonlySignal<T> & { dispose }`)** : tuple choisi pour éviter de muter l'instance Preact Signal (collage d'une prop `dispose` via `Object.assign`) et pour découpler signal et lifecycle — le signal peut voyager seul vers un sous-composant sans embarquer son contrôle de fin de vie.
...
- **OSS community files** : doter le repo des fichiers de gouvernance attendus par les contributeurs externes pour faire passer le score « Community Standards » de GitHub à 100 %. Livrables : `CONTRIBUTING.md` racine (workflow PR fork → branch → PR vers `main`, commande unique de validation locale `pnpm verify` qui mappe le job `ci` de [.github/workflows/ci.yml](.github/workflows/ci.yml), convention **conventional commits** déjà en vigueur dans l'historique — `feat(core)`, `chore(deps)`, `docs(spec)` — section dédiée **changesets** expliquant `pnpm changeset` + le choix patch/minor/major + rappel du mode `fixed` qui bumpe les 4 packages publics ensemble, section reporting pointant vers `.github/ISSUE_TEMPLATE/` et `SECURITY.md`, pointage vers `spec.md` pour le contexte design), `CODE_OF_CONDUCT.md` racine (Contributor Covenant 3.0, texte officiel copié intégralement, contact maintainer = handle `@clemparpa` GitHub, aucun email exposé), `SECURITY.md` racine (reporting via GitHub Private Vulnerability Reporting — lien direct vers `/security/advisories/new`, politique de support = HEAD de `main` uniquement, SLA acknowledgement < 7 jours, scope explicite : vulns des deps relèvent de l'upstream et sont déjà couvertes par Dependabot + `pnpm audit` côté CI cf. §8.2), `.github/ISSUE_TEMPLATE/bug_report.yml` (format YAML moderne, `labels: ["bug"]`, `title: "[Bug] "`, champs structurés description / steps / expected / actual / Node version / pnpm version / OS dropdown / contexte additionnel, checkbox « I have searched existing issues »), `.github/ISSUE_TEMPLATE/feature_request.yml` (`labels: ["enhancement"]`, `title: "[Feature] "`, motivation / solution / alternatives / contexte, même checkbox de pré-recherche), `.github/ISSUE_TEMPLATE/config.yml` (`blank_issues_enabled: false`, `contact_links` vers Discussions et `/security/advisories/new` — les 404 sur ces liens disparaissent une fois la story « README polish + Settings GitHub » livrée), `.github/pull_request_template.md` version détaillée (Summary, Motivation, Changes, How to test, Breaking changes, Changeset added?, checklist de 4-5 cases). Aucun fichier de code touché, `pnpm verify` reste vert. Critère d'acceptation : Insights → Community Standards affiche tous les items cochés (description, README, CoC, Contributing, License, issue templates, PR template) — sauf les 2 qui dépendent de la story suivante (Discussions activées + PVR activé côté Settings).

- **README polish + Settings GitHub** : refondre le README racine (aujourd'hui 1 ligne `# signal-store`) en page d'entrée pour la lib npm, et finaliser les métadonnées GitHub. **Pas de badge « Use this template »** — ce repo distribue une lib npm publiée, ce n'est pas un starter. Badges en tête (style shields.io flat) : version npm du core `@fluch/signal-store`, bundle size (badge bundlephobia ou pkg-size sur le core), statut CI natif GitHub Actions sur `main`, License MIT, Node ≥22, lien doc Starlight. Corps du README dans cet ordre : description 1 paragraphe → Quickstart `pnpm add @fluch/signal-store @preact/signals-core rxjs` + exemple condensé 15-20 lignes (signalStore + withState + withMethods + patchState) → section Packages (4 lignes, 1 par package publié avec rôle : core, entities, react, devtools) → lien vers le site Starlight pour le reste de l'API → section Contributing pointant vers `CONTRIBUTING.md` et `.github/ISSUE_TEMPLATE/` → section Community (Discussions, Code of Conduct, contact maintainer `@clemparpa`) → License. Checklist d'actions côté Settings GitHub (manuelles dans l'UI, traçables en checkboxes dans la PR de livraison) : renseigner Description + Website (= lien doc Starlight) dans le panneau About, ajouter les topics `react state-management signals preact-signals rxjs ngrx typescript monorepo npm-package`, activer Discussions (Settings → General → Features), activer Private Vulnerability Reporting (Settings → Security & analysis), **renforcer la branch protection sur `main`** — aujourd'hui seul 1 reviewer est requis : ajouter les 2 status checks CI required (`ci` qui couvre `pnpm verify` = lint + typecheck + test + build, et `audit` qui bloque high/critical), require linear history, require conversation resolution. Critère d'acceptation final : page Insights → Community Standards à 100 %, panneau About complet, une PR factice avec CI rouge bloquée par la branch protection.

- **Release process — documentation seulement** : le release flow automatisé est **déjà en place** ([.github/workflows/release.yml](.github/workflows/release.yml) + `changesets/action`, CHANGELOG par package auto-généré sous `packages/*/CHANGELOG.md`, tags individuels par package au publish, GitHub Releases auto-créées). Pas de `CHANGELOG.md` racine — les CHANGELOG par package suffisent et reflètent la granularité réelle du monorepo (mode `fixed` aligne juste les versions, pas l'historique des changes). Reste à documenter dans `CONTRIBUTING.md` (livré par la story « OSS community files » — section dédiée à ajouter en même temps si possible) : (a) la **policy semver** appliquée — `major` = breaking sur l'API publique (rare, annoncé en avance via discussion), `minor` = nouvelle capacité ou nouvelle option non-breaking, `patch` = fix / docs / internals — et son interaction avec le mode `fixed` (un patch sur n'importe lequel des 4 packages bumpe automatiquement les 3 autres au même cran sans diff utile sur les non-modifiés ; tradeoff accepté pour une matrice de compat plate `^1.0.0` côté consommateur, cf. décision livraison §11 « Release 1.0.0 unifiée ») ; (b) le **release flow** côté opérateur — PR `Version Packages` ouverte automatiquement par `changesets/action` à chaque merge sur `main` contenant un changeset, relue, mergée, `release.yml` publie sur npm + crée les tags + crée les GitHub Releases. Story légère (édition d'une seule section dans `CONTRIBUTING.md`) — peut être livrée en même temps que « OSS community files » si la rédaction de `CONTRIBUTING.md` se fait en une passe.

- **Governance avancée (optionnelle)** : à n'exécuter que si le projet attire un trafic externe significatif (signal : 3+ PRs externes/mois) **ou** si le maintainer ouvre un canal de sponsoring. `.github/CODEOWNERS` avec règle catch-all `* @clemparpa` au départ — affinement par dossier (`/packages/core/`, `/apps/docs/`) reporté tant qu'il n'y a pas de co-maintainers (sinon c'est du bruit qui ralentit les PRs sans gain). Activation côté Settings de la branch protection rule « Require review from Code Owners » sur `main` (assure que toute PR est auto-routée vers `@clemparpa` même si quelqu'un l'oublie en reviewer manuel). `.github/FUNDING.yml` pour exposer le bouton « Sponsor » en tête du repo : pré-requis = compte GitHub Sponsors actif côté `@clemparpa`, sinon le bouton n'apparaît pas (ne pas déclarer dans le fichier des plateformes vides — ça pollue sans rien afficher). Rien à livrer sans déclencheur explicite ; story laissée volontairement ouverte.
